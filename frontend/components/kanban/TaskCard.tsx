...
import { MessageSquare, Clock3, AlertCircle } from "lucide-react";
...
const getStatusBadge = (status: TaskStatus) => {
  ...
  if (status === "PENDING_PROGRESS") return "Pending Progress";
  if (status === "PENDING_DONE") return "Pending Done";
  return null;
};
    ...
    <Draggable draggableId={task.id} index={index} isDragDisabled={isDragDisabled}>
      {(provided, snapshot) => (
        <article
          ...
          onClick={onClick}
          className={`mb-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900 border-l-4 ${priorityColor[task.priority]} ${snapshot.isDragging ? "rotate-[1deg] scale-[1.03] shadow-xl" : ""} ${badge ? "opacity-80" : ""}`}
        >
